        ...
        const INT *hex, size_t hex_num)
{
    ...
    os << "CELLS " << hex_num + tet_num << " " << hex_num*9 + tet_num * 5 << "\n";
    for(size_t i = 0; i < tet_num; ++i)
        os << 4 << "  "
           << tet[i*4+0] << " " << tet[i*4+1] << " "
           << tet[i*4+2] << " " << tet[i*4+3] << "\n";
        ...
        const INT *hex, size_t hex_num)
{
    ...
    os << "CELLS " << hex_num + tet_num + pag_num << " "
       <<hex_num*9 + tet_num * 5  + pag_num * 6<< "\n";
    for(size_t i = 0; i < tet_num; ++i)
        os << 4 << "  "
           << tet[i*4+0] << " " << tet[i*4+1] << " "
           << tet[i*4+2] << " " << tet[i*4+3] << "\n";
    for(size_t i = 0; i < pag_num; ++i)
        os << 5 << "  "
           << pag[i*5+0] << " " << pag[i*5+1] << " "
           << pag[i*5+2] << " " << pag[i*5+3] << " "<<pag[i*5+4] <<"\n";
    for(size_t i = 0; i < hex_num; ++i)
        os << 8 << "  "
           << hex[i*8+7] << " " << hex[i*8+5] << " "
           << hex[i*8+4] << " " << hex[i*8+6] << " "
           << hex[i*8+3] << " " << hex[i*8+1] << " "
           << hex[i*8+0] << " " << hex[i*8+2] << "\n";
    os << "CELL_TYPES " << hex_num + tet_num + pag_num<< "\n";
    for(size_t i = 0; i < tet_num; ++i)
        os << 10 << "\n";
    for(size_t i = 0; i < pag_num; ++i)
        os << 14 << "\n";
